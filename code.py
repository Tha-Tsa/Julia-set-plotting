...
def plot_julia_set(coef, width, height, real_min, real_max, imag_min, imag_max, max_iter):
    ...
    #this is an array with a number corresponding to each pixel on the screen. Pixels are determined by width and height and each number between 0 and 1 shows what percentage of max iterations each point used before becoming larger than 2. Points in the filled Julia set always use all iterations. 
